  ...
  try {
    ...
    if (mode === "SELL") {
      ...
      if (existingHolding.qty < qty) {
        return res.status(400).json({
          message: `Insufficient holdings. You own ${existingHolding.qty} shares, but trying to sell ${qty}`,
        });
      }
    ...
    res.json({ message: "Order executed successfully!", order: newOrder });
  } catch (error) {
...
app.get("/allOrders", authenticateToken, async (req, res) => {
  try {
    let allOrders = await OrdersModel.find({ userId: req.user.userId }).sort({ createdAt: -1 });
    res.json(allOrders);
  } catch (error) {
